...
fs.mkdir("./poem", function (err) {
    if (err) throw err;
    console.log(__dirname + "\\poem folder created");
});

for (var i = 0; i < poem.length; i++) {
    fs.writeFile("./poem/poem-" + i + ".txt" , poem[i], function (err) {
        if (err) throw err;
        console.log(__dirname + "\\poem\\poem-" + i + ".txt created");
    });
}
